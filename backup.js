...
app.controller("eventSum", function($scope, $rootScope, $interval, $http) {
    ...
    $scope.$watch('reportType.id', function(newValue, oldValue) { 
        ...
        if (newValue == 2) {
            var x = newValue.toString();
            $scope.reportTypeAPIparam = "scalar";
        };
        if (newValue == 3) {
            var x = newValue.toString();
            $scope.reportTypeAPIparam = "enumeration";
        };
    }); 
    ...
    */
    ...
    $scope.$watchGroup(['reportTypeAPIparam','reportFreqAPIparam', 'reportStyleAPIparam','reportDateAPI'], function(newValues, oldValues) {
        if (newValues[2] == "summary") {
            $scope.apiEndpoint = "http://ec2-52-5-122-115.compute-1.amazonaws.com:8080/WebAnalytics/v1.0/" + newValues[2] + "/" + newValues[0] + "/period" + newValues[1] + "&" + newValues[3]; 
        };
        if (newValues[2] == "adhoc") {
            $scope.apiEndpoint = "http://ec2-52-5-122-115.compute-1.amazonaws.com:8080/WebAnalytics/v1.0/" + newValues[2] + "/" + newValues[0] + newValues[1] + "&" + newValues[3]; 
        };
